...
"""
The following is a demonstration of how to use PYCCF.
...
interp = 2.0  # Interpolation time step (days). Must be less than the average cadence of the observations, but too small will introduce noise.
...
centau_loerr = centau - stats.scoreatpercentile(tlags_centroid, 100.0 - perclim)
...
peaktau_loerr = centau - stats.scoreatpercentile(tlags_peak, 100.0 - perclim)
...
##########################################
output_file_folder = './'
with open(output_file_folder + 'sample_centtab_yap.dat', 'w') as centfile:
    for m in range(np.size(tlags_centroid)):
        centfile.write('%5.5f\n' % (tlags_centroid[m]))

with open(output_file_folder + 'sample_peaktab_yap.dat', 'w') as peakfile:
    for m in range(np.size(tlags_peak)):
        peakfile.write('%5.5f\n' % (tlags_peak[m]))

with open(output_file_folder + 'sample_ccf_yap.dat', 'w') as ccf_file:
    for m in range(np.size(lag)):
